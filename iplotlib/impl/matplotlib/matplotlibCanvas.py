...
class MatplotlibCanvas:
    ...
    def process_iplotlib_plot(self, canvas: Canvas, plot: Plot, griditem):
        ...
        def axis_update_callback(axis):
            ...
            # logger.info("Affected axes", [id(a) for a in affected_axes])
                            ...
                            for stack_axis in range_axis:
                                ...
                                if axis_index == 0:
                                    ...
                                    if isinstance(stack_axis, RangeAxis):
                                        a_min, a_max = stack_axis.begin, stack_axis.end
                                    else:
    ...
    def process_iplotlib_axis(self, canvas, plot, axis_idx, mpl_axes, axis=None):
        ...
        if isinstance(axis, LinearAxis):
            if axis.is_date:
                ...
                print(F"** X AXIS LOCATOR: {mpl_axis.get_major_locator()} x space: {mpl_axis.get_tick_space()}")
    ...
    def refresh_signal(self, signal):
        ...
        def autosize_x_from_data(axes, plot, signal, data):
            if all(e is not None for e in [axes, plot, data, plot.axes]):
                if len(plot.axes) > 0 and len(data) > 0:
                    if len(data[0]) > 0:
                        if isinstance(plot.axes[0], RangeAxis):
                            x_range = NanosecondHelper.mpl_get_lim(axes, 0)
                            if plot.axes[0].begin is None:
                                x_range = data[0][0], x_range[1]
                            if plot.axes[0].end is None:
                                x_range = x_range[0], data[0][-1]

                            logger.info(F"AUTOSIZE X {plot.axes[0]} x range {x_range}")
                            NanosecondHelper.mpl_set_lim(axes, 0, x_range)
                    else:
                        logger.info(F"AUTOSIZE: No data!")
                        if hasattr(signal, 'get_ranges'):
                            ranges = signal.get_ranges()
                            if ranges is not None and len(ranges)>0:
                                NanosecondHelper.mpl_set_lim(axes, 0, ranges[0])

        def autosize_x_from_data2(plot, signal, data, axis_idx=0):
            data_range = get_data_range(data, axis_idx)
            signal_ramge = None
            axis_range = None
            ...
            if hasattr(signal, 'get_ranges'):
                ...
                if ranges is not None and len(ranges)>axis_idx:
                    signal_ramge = ranges[axis_idx]

            if plot is not None and plot.axes is not None and len(plot.axes) > axis_idx:
                axis = plot.axes[axis_idx]
                if isinstance(axis, RangeAxis):
                    axis_range = [axis.begin, axis.end]
        ...
        def update_line(mpl_axes, signal):
            ...
            autosize_x_from_data(mpl_axes, mpl_axes._plot, signal, signal_data,)
        ...
        # mpl_axes = self.mpl_axes.get(id(signal))
        # logger.info("Refresh signal", signal)

        # if mpl_axes is not None:
            # plot = mpl_axes._plot

            # canvas = mpl_axes._canvas
            # style = self.get_signal_style(signal, canvas=canvas, plot=plot)
            # step = style.pop('step', None)

            # shapes = self.mpl_shapes.get(id(signal))
            # data = NanosecondHelper.mpl_axes_transform_data(mpl_axes, signal.get_data())
            # logger.info("Transformed", data[0])
            # plot = mpl_axes._plot

            # if shapes is not None:
                # shapes[0][0].set_xdata(data[0])
                # shapes[0][0].set_ydata(data[1])

                # if hasattr(signal, "envelope") and signal.envelope:
                #     shapes[1][0].set_xdata(data[0])
                #     shapes[1][0].set_ydata(data[2])
                #     shapes[2].remove()
                #     shapes.pop()
                #     shapes.append(mpl_axes.fill_between(data[0], data[1], data[2], alpha=0.3, color=shapes[1][0].get_color(), step=step))
                ...
                #             yaxis.set_label_text(label)
                #
                # current_renderer = mpl_axes.get_renderer_cache()
                #
                # if current_renderer:
                #     if plot.axes[0].follow:
                #         mpl_window = 0 if plot.axes[0].window is None else plot.axes[0].window * 1000000000
                #
                #         x_min, x_max = mpl_axes.get_xbound()
                #         margin_x = (x_max - x_min) * mpl_axes.margins()[0]
                #         x_max -= margin_x
                #         x_max = data[0][-1] if data[0][-1] > x_max else x_max
                #         mpl_axes.set_xlim(left=x_max - mpl_window, right=x_max)
                #
                #         cur_ymin, cur_ymax = mpl_axes.get_ybound()
                #         margin_y = (cur_ymax - cur_ymin) * mpl_axes.margins()[1]
                #         y_min, y_max = np.min(data[1]), np.max(data[1])
                #
                #         # logger.info("CUR", cur_ymin, cur_ymax, "DATA", y_min, y_max)
                #         if plot._has_data:
                #             y_min = y_min if y_min < cur_ymin + margin_y else cur_ymin + margin_y
                #             y_max = y_max if y_max > cur_ymax - margin_y else cur_ymax - margin_y
                #
                #         # logger.info("\tSET", y_min-margin_y, y_max+margin_y)
                #
                #         mpl_axes.set_ybound(lower=y_min - margin_y, upper=y_max + margin_y)
                #
                #         mpl_axes.figure.draw(current_renderer)
                #     else:
                #         self._redraw_in_frame_with_grid(mpl_axes)
                #
                #     mpl_axes.figure.canvas.update()  # else:  #     mpl_axes.figure.canvas.update()
            ...
            # else:
                ...
                # autosize_x_from_data(mpl_axes, plot, data)
                # line = draw_function(data[0], data[1], **params)
...
# FIXME: Crosshair seems to stay when plotted on axes with no data and switched to another tool
...
class NanosecondHelper:
    ...
    def mpl_set_lim(mpl_axes, axis_idx, limits):
        ...
        if not hasattr(mpl_axis, '_offset'):
            ...
            logger.info(F"SET_XLIM axis {axis_idx} does not have offset new={new_offset}")
            if new_offset is not None:
                mpl_axis._offset = new_offset

        if hasattr(mpl_axis, '_offset'):
            logger.info(F"SET_XLIM axis {axis_idx} DOES  have offset={mpl_axis._offset}")
        ...
        if axis_idx == 0:
            logger.info(F"***DEBUG begin={begin} (type{begin}) end={end} ({type(end)})")
            if begin == end:
                begin = end-1
