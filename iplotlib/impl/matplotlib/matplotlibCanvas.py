...
class MatplotlibParser(BackendParserBase):
    ...
    def do_mpl_line_plot(self, signal: Signal, mpl_axes: MPLAxes, data: List[BufferObject]):
        ...
        if isinstance(signal, SignalXY):
            ...
            if isinstance(plot, PlotContourWithSlider):
                plot_lines = self.do_mpl_line_plot_contour_slider(signal, mpl_axes, plot, data[0], data[1], data[2])
            else:
    ...
    def process_ipl_plot_xy_slider(self, plot_with_slider: PlotXYWithSlider | PlotContourWithSlider,
        ...
        # In case of PlotWithSlider, create a vertical layout with `stack_sz` + 1 (slider) rows and 1 column
        # inside grid_item
    ...
    def process_ipl_signal(self, signal: Signal):
        ...
        if isinstance(signal, SignalXY):
            if mpl_axes.get_lines() and mpl_axes.get_lines()[0].get_marker() == 'None':
                return
