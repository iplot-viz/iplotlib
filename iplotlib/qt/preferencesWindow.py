...
from PyQt5.QtWidgets import QApplication, QCheckBox, QColorDialog, QComboBox, QDataWidgetMapper, QFormLayout, QLabel, \
    QLineEdit, QMainWindow, QPushButton, QSizePolicy, QSpinBox, QSplitter, QStackedWidget, QTreeView, QVBoxLayout, \
    QWidget
...
class CanvasItemModel(QStandardItemModel):
    ...
    def __init__(self, canvas=None, auto_names=False):
        super().__init__()
    ...
    def create_tree(self):
        ...
        def addAxis(label, ipl_axis, plotItem):
                    ...
                    if plot:
                        ...
                        for axis_idx, axis in enumerate(plot.axes):
                            if isinstance(axis, Collection):
                                if len(axis) == 1:
                                    addAxis(F"Axis {self.axis_names[axis_idx]}", axis[0], plotItem)
                                else:
                                    for subaxis_idx, subaxis in enumerate(axis):
                                        addAxis(F"Axis {self.axis_names[axis_idx]}{subaxis_idx}", subaxis, plotItem)
                            else:
                                addAxis(F"Axis {self.axis_names[axis_idx]}", axis, plotItem)
