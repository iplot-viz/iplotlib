...
class RulerPyQtGraphEndToEndTest(unittest.TestCase):
    ...
    def test_default_font_autocontrasts_with_background(self):
        ...
        self.assertEqual(backend._text_color_for(backend.color), 'black')   # light ruler
        self.assertEqual(backend._text_color_for('green'), 'white')         # value tag bg
    ...
    def test_ruler_shows_one_value_label_per_signal(self):
        ...
        label = backend.value_labels[0]
        self.assertTrue(label.isVisible())
        x = np.linspace(0, 10, 50)
        nearest = int(np.argmin(np.abs(x - 2.5)))
        self.assertEqual(label.textItem.toPlainText(), f"{np.sin(x)[nearest]:.6g}")
