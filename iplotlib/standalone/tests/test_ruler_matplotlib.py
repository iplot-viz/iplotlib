...
class RulerMatplotlibEndToEndTest(unittest.TestCase):
    ...
    def test_toggle_ruler_label_hides_the_ruler_tags_but_not_the_lines(self):
        ...
        self.assertTrue(backend.h_line.get_visible())
        self.assertTrue(backend.value_labels[0].get_visible())
    ...
    def test_ruler_shows_one_value_label_per_signal(self):
        ...
        # Same text the pyqtgraph label and the ruler window cell render.
        self.assertEqual(annotation.get_text(), f"{np.sin(x)[nearest]:.6g}")
...
class RulerMatplotlibFocusTest(unittest.TestCase):
    ...
    def _stacked_plots_with_a_ruler_each(self, shared: bool):
        c = Canvas(3, 1, title="focus_mpl", shared_x_axis=shared)
        x = np.linspace(0, 10, 50)
        for k in range(3):
            p = PlotXY()
            ...
            p.add_signal(s)
            c.add_plot(p, 0)
        self.widget = QtMatplotlibCanvas(canvas=c)
    ...
    def _two_plots_with_archived_data_and_a_ruler_each(self):
        ...
        c = Canvas(2, 1, title="focus_ts_mpl", shared_x_axis=True)
