"""End-to-end tests for the frozen-Crosshair feature on the matplotlib backend.

Mirrors test_crosshair_pyqtgraph.py so both backends behave identically from the
perspective of Plot.crosshairs / parser._crosshairs / the crosshair window, and
pins the crosshair-specific contract (per-signal value dict, solid lines).
"""
...
def _build_canvas() -> Canvas:
    c = Canvas(1, 1, title="crosshair_test_mpl")
...
class CrosshairMatplotlibEndToEndTest(unittest.TestCase):
    @classmethod
    ...
    def test_freeze_populates_model_parser_and_window(self):
        ...
        self.assertIsInstance(parser_crosshairs[0], iplotMplCrosshair)
    ...
    def test_signal_values_carry_color_and_axis_text(self):
        self.widget._add_crosshair_at(self.impl_plot, self.plot, 2.5, 0.5)
        sv = self.widget._crosshair_window._rows[0]['signal_values']
        # Colour (for column tinting) and the axis-formatted text (matches the
        # on-plot value label) travel with each signal entry.
        self.assertTrue(all(sv[label].get('color') for label in ('s1', 's2')), sv)
        self.assertTrue(all(sv[label].get('text') for label in ('s1', 's2')), sv)

    def test_frozen_crosshair_uses_solid_lines(self):
        self.widget._add_crosshair_at(self.impl_plot, self.plot, 2.5, 0.5)
        artist = self.widget._parser.get_crosshairs(self.impl_plot)[0]
        self.assertEqual(artist.v_line.get_linestyle(), '-')
    ...
    def test_toggle_visibility_updates_model_and_backend(self):
        ...
        self.assertFalse(self.widget._parser.get_crosshairs(self.impl_plot)[0].v_line.get_visible())
